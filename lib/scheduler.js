...
export async function runScheduler() {
  ...
  try {
    ...
    for (const update of pendingUpdates) {
      try {
        // Find the channel connection associated with this video's userEmail, channelId, or fallback
        let channel = null;
        if (update.userEmail) {
        ...
        if (!channel) {
          throw new Error(`No connected YouTube channel found in the system.`);
        }
